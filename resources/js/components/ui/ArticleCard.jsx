...
export const ArticleCard = ({
    ...
    isBreaking = false,
    viewCount,
    variant = 'standard', // 'featured' | 'compact' | 'standard'
    className = ''
}) => {
    ...
    const typeColors = {
        'News': '#ef4444', 'Blog': '#6366f1', 'Industry Guide': '#f59e0b',
        'Tips & Tricks': '#10b981', 'How-To': '#287FBA', 'Checklist': '#8b5cf6',
        'Guide': '#4A9AD4', 'FAQ': '#ec4899', 'Best Practice': '#14b8a6',
        'Industry Trend': '#f97316', 'Beginner Guide': '#84cc16',
        'Educational Resource': '#0ea5e9', 'Expert Insight': '#7c3aed',
    };

    return (
        /* ── Entire card is one big Link ─────────────────── */
        <Link
            href={`/ideas/${slug}`}
            className={`
            ...
            {heroImage ? (
                ...
                >
                    ...
                    <div className="absolute bottom-2 left-2 flex gap-1">
                        {isBreaking && !heroImage && (
                            <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-red-600 text-white text-[9px] font-black uppercase tracking-wider">
                                <Zap className="w-2.5 h-2.5" /> Breaking
                            </span>
                        )}
                        {isTrending && !isBreaking && !heroImage && (
                            <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-orange-500 text-white text-[9px] font-black uppercase tracking-wider">
                                <Flame className="w-2.5 h-2.5" /> Trending
                            </span>
                        )}
                ...
                <div className="flex items-center gap-2 text-[11px] font-bold text-[#466071] shrink-0">
                    {viewCount && (
                        <span className="flex items-center gap-0.5 text-slate-500">
                            <Eye className="w-3 h-3 text-[#4A9AD4]" /> {typeof viewCount === 'number' ? viewCount.toLocaleString() : viewCount}
                        </span>
                    )}
